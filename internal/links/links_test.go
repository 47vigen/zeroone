...
func TestVLESSIncludesConfiguredClientEndpoint(t *testing.T) {
	...
	for _, want := range []string{"edge.example.com:443", "security=tls", "sni=edge.example.com", "host=edge.example.com", "path=%2Fapi%2Fv1%2Fevents", "type=xhttp", "mode=stream-up"} {
		if !strings.Contains(cdn.URL, want) {
			t.Fatalf("CDN URL missing %q: %s", want, cdn.URL)
		}
	}
	if strings.Contains(got[0].URL, "edge.example.com") {
...
func TestVLESSEmitsSplitHTTPCompatLinks(t *testing.T) {
	...
	for _, want := range []string{"type=splithttp", "host=203.0.113.1", "security=none", "path=%2Fapi%2Fv1%2Fevents", "mode=stream-up"} {
	...
	for _, want := range []string{"type=splithttp", "host=edge.example.com", "security=tls", "sni=edge.example.com", "mode=stream-up"} {
