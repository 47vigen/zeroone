    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="panel panel-pad">
          ...
          <div className={`pill ${summary?.allow_apply ? "pill-ok" : "pill-bad"}`}>
            <span className="dot" />
            {summary?.allow_apply ? "Apply enabled" : "Apply locked"}
          </div>
        </div>

        <div className="panel panel-pad">
          <h2 className="mb-3 text-sm font-semibold tracking-tight">Failover</h2>
          {failover ? (
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="kpi-label">Enabled</dt>
                <dd>{failover.enabled ? "Yes" : "No"}</dd>
              </div>
              <div>
                <dt className="kpi-label">Probe</dt>
                <dd className="font-mono text-xs">
                  {failover.probe_ip}:{failover.probe_port}
                </dd>
              </div>
              <div>
                <dt className="kpi-label">Cooldown</dt>
                <dd>{failover.cooldown_seconds}s</dd>
              </div>
            </dl>
          ) : (
  ...
  return (
    <div className="panel">
      <div className="flex items-center justify-between border-b border-border px-5 py-3 dark:border-border-dark">
        ...
        <button className="btn text-xs" onClick={() => setCreating(true)}>
      ...
      <div className="divide-y divide-border dark:divide-border-dark">
        ...
        {items.map((a) => (
          <div key={a.username} className="flex items-center gap-3 px-5 py-3 text-sm">
            ...
            <button className="btn text-xs" onClick={() => setPwTarget(a.username)}>
              Change password
            </button>
            <button
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            disabled={pending || password.length < 8 || (!fixedUsername && !username)}
          >
            {submitLabel}
          </button>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-[1fr,1fr,1fr] gap-2">
            <div>
              ...
              <input
                ...
                max="65535"
                value={form.port}
                onChange={(e) => setForm({ ...form, port: parseInt(e.target.value, 10) || 443 })}
                required
              />
            </div>
            <div>
              ...
              <select
                className="input"
                value={form.network}
                onChange={(e) => setForm({ ...form, network: e.target.value as "ws" | "xhttp" })}
              >
            ...
            <div>
              ...
              <button
                type="button"
                className={`btn w-full justify-center ${form.tls ? "btn-primary" : ""}`}
          ...
          {form.network === "xhttp" && (
            <div>
              ...
              <select
                className="input"
                value={form.mode ?? "stream-up"}
                onChange={(e) =>
                  setForm({ ...form, mode: e.target.value as ClientEndpoint["mode"] })
                }
              >
          ...
          {endpoints.map((ep) => (
            ...
            >
              ...
              <div className="flex justify-end gap-1">
                <button className="btn px-2 py-1" onClick={() => edit(ep)} title="Edit">
                  <Globe2 size={12} />
                </button>
                <button
                  className="btn btn-danger px-2 py-1"
                  onClick={() => remove(ep.name)}
                  title="Delete"
                >
                  <Trash2 size={12} />
                </button>
...
function EndpointStatus({ health, enabled }: { health?: ClientEndpointHealth; enabled: boolean }) {
  ...
  if (health.ok && health.landing_ok) {
    return (
      <div className="pill pill-ok mt-1">
        <span className="dot" />
        Healthy
      </div>
    );
  ...
  return (
    <div className="pill pill-bad mt-1" title={health.error || "Endpoint failed"}>
      <span className="dot" />
      Down
    </div>
  );
...
function TokensPanel() {
  ...
  async function createToken(id: string, scope: string) {
    try {
      const res = await post<{ ok: boolean; id: string; token: string }>("/api/tokens", {
        id,
        scope,
      });
  ...
  return (
    <div className="panel">
      <div className="flex items-center justify-between border-b border-border px-5 py-3 dark:border-border-dark">
        ...
        <button className="btn btn-primary text-xs" onClick={() => setCreating(true)}>
      ...
      <div className="divide-y divide-border dark:divide-border-dark">
        ...
        {(data?.tokens ?? []).map((t) => (
          ...
          >
            ...
            <button className="btn btn-danger text-xs" onClick={() => removeToken(t.id)}>
      ...
      {newPlain && (
        <RevealTokenDialog
          id={newPlain.id}
          token={newPlain.token}
          onClose={() => setNewPlain(null)}
        />
      )}
    </div>
  );
...
function CreateTokenDialog({
  ...
  onCancel: () => void;
  onCreate: (id: string, scope: string) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/40 p-4" onClick={onCancel}>
      ...
      >
        <h3 className="mb-3 flex items-center gap-2 text-base font-semibold">
          <KeyRound size={16} /> New API token
        </h3>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div>
          ...
          <label className="mb-1 block text-xs">
            Secret <span className="text-muted">(HMAC-SHA256, leave blank to keep)</span>
          </label>
          <input
            type="password"
            className="input mb-2"
            placeholder={data?.notifications.webhook.secret_set ? "••••••• (set)" : ""}
            value={webhookSecret}
            onChange={(e) => setWebhookSecret(e.target.value)}
          />
          <label className="mb-1 block text-xs">Events</label>
          <div className="flex flex-wrap gap-1.5">
            {EVENT_KINDS.map((k) => (
              <button
                key={k}
                type="button"
                className={`pill text-xs ${webhookEvents.includes(k) ? "pill-ok" : "text-muted"}`}
                onClick={() => toggle(webhookEvents, setWebhookEvents, k)}
              >
                <span className="dot" />
                {k}
              </button>
        ...
        <div>
          ...
          <label className="mb-1 block text-xs">
            Bot token <span className="text-muted">(leave blank to keep)</span>
          </label>
          <input
            type="password"
            className="input mb-2"
            placeholder={data?.notifications.telegram.bot_token_set ? "••••••• (set)" : ""}
            value={tgBotToken}
            onChange={(e) => setTgBotToken(e.target.value)}
          />
          <label className="mb-1 block text-xs">Chat ID</label>
          <div className="mb-2 flex gap-2">
            ...
            />
            <DiscoverChatsButton
              botSet={data?.notifications.telegram.bot_token_set ?? false}
              onPick={(id) => setTgChatID(String(id))}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {EVENT_KINDS.map((k) => (
              <button
                key={k}
                type="button"
                className={`pill text-xs ${tgEvents.includes(k) ? "pill-ok" : "text-muted"}`}
                onClick={() => toggle(tgEvents, setTgEvents, k)}
              >
                <span className="dot" />
                {k}
              </button>
...
function DiscoverChatsButton({
  ...
  async function fetchChats() {
    ...
    setError("");
    try {
      const res = await api<{ ok: boolean; chats: TelegramChat[]; hint: string }>(
          ...
          <div className="panel panel-pad w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <h3 className="flex items-center gap-2 text-base font-semibold">
                <MessageCircle size={14} /> Find Telegram chat
              </h3>
                ...
                {chats.map((c) => (
                  <button
                    ...
                    onClick={() => {
                      onPick(c.chat_id);
                      setOpen(false);
                      toast.show(`Chat ID set: ${c.chat_id}`, "ok");
                    }}
                  >
