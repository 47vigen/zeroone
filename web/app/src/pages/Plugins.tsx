...
function RelayStatusCard({
  ...
  function runTest() {
    test.mutate(undefined, {
      onSuccess: (res) => {
        ...
        toast.show(
          p.ok
            ? `Relay OK · ${p.latency_ms ?? "?"}ms via ${p.target ?? (cfg.health_probe || cfg.defaults.health_probe)}`
            : `Relay failed: ${p.error || p.status}`,
          p.ok ? "ok" : "bad",
        );
      },
  ...
  return (
    <section className="panel">
      ...
      <div className="grid grid-cols-2 gap-3 px-5 py-4 text-sm md:grid-cols-4">
        <Stat label="Listen" value={status?.listen || cfg.listen || cfg.defaults.listen} mono />
        <Stat
          label="Sites routed"
          value={`${status?.enabled_sites ?? cfg.sites.filter((s) => s.enabled).length}/${cfg.sites.length}`}
        />
        <Stat
          label="Latency"
        ...
        <Stat
          ...
          tone={status?.binary_found ? "ok" : "bad"}
        />
        <Stat
          label="Managed"
          value={managed ? "supervised" : "external"}
          tone={managed ? "ok" : "muted"}
        />
        <Stat label="Restarts" value={String(status?.restart_count ?? 0)} mono />
        <Stat
          label="Last probe"
          value={
            status?.last_probe ? relativeTime(new Date(status.last_probe).getTime() / 1000) : "—"
          }
        />
      </div>
      {status?.last_error && (
      ...
      <div className="flex flex-wrap gap-2 border-t border-border px-5 py-3 dark:border-border-dark">
        <button
          ...
          onClick={runTest}
          disabled={test.isPending || !cfg.enabled}
        >
        ...
        <button
          ...
          onClick={doRestart}
          disabled={restart.isPending || !managed}
        >
...
function RelayConfigForm({ data }: { data: RelayConfigView }) {
  ...
  function save() {
    const patch: RelayConfigPatch = {
      ...
      deployment_ids: form.deployment_ids
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean),
      inbound_tags: form.inbound_tags
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean),
      binary: form.binary,
      ...
      <div className="space-y-3 px-5 py-4 text-sm">
        <Field label={`Listen address (default ${data.defaults.listen})`}>
          <input
            ...
            placeholder={data.defaults.listen}
          />
        </Field>
        ...
        <Field label={`Binary path (default ${data.defaults.binary})`}>
          <input
            ...
            placeholder={data.defaults.binary}
          />
        </Field>
        <Field label={`Generated config path (default ${data.defaults.config_path})`}>
          <input
            ...
            placeholder={data.defaults.config_path}
          />
        </Field>
        <Field label="systemd unit (optional — leave blank to spawn as child)">
          <input
            className="input"
            value={form.systemd_unit}
            onChange={(e) => setForm({ ...form, systemd_unit: e.target.value })}
            placeholder="mhrv-rs.service"
          />
        </Field>
        <Field label={`Outbound tag (default ${data.defaults.outbound_tag})`}>
          <input
            className="input"
            value={form.outbound_tag}
            onChange={(e) => setForm({ ...form, outbound_tag: e.target.value })}
            placeholder={data.defaults.outbound_tag}
          />
        </Field>
        <Field label={`Health probe target (default ${data.defaults.health_probe})`}>
          <input
            className="input"
            value={form.health_probe}
            onChange={(e) => setForm({ ...form, health_probe: e.target.value })}
            placeholder={data.defaults.health_probe}
          />
        </Field>
        <Field label="Notes">
          <textarea
