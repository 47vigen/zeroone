          ...
          {socks.map((s) => (
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  onClick={async () => {
                    const url =
                      s.links?.[0]?.url ??
                      `socks5://${s.username}:${s.password}@${summary.data?.public_ip ?? s.listen}:${s.port}`;
                    const ok = await copyText(url);
                    toast.show(
                      ok ? "Copied" : "Copy failed — select text manually",
                      ok ? "ok" : "bad",
                    );
                  }}
                >
              ...
              <div className="flex items-center justify-end gap-1">
                <button className="btn px-2 py-1" onClick={() => setEditing(s)} title="Edit">
                  <Pencil size={12} />
                </button>
                <button
                  className="btn btn-danger px-2 py-1"
                  onClick={() => remove(s.name, s.username)}
                  title="Delete"
                >
                  <Trash2 size={12} />
                </button>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="panel">
          ...
          <div className="max-h-72 space-y-1 overflow-auto px-5 py-3 font-mono text-xs">
            {(summary.data?.direct_domains ?? []).map((d) => (
              <div key={d}>{d}</div>
            ))}
        ...
        <div className="panel">
          ...
          <div className="max-h-72 space-y-1 overflow-auto px-5 py-3 font-mono text-xs">
            {[...(summary.data?.block_domains ?? []), ...(summary.data?.manual_blocks ?? [])].map(
              (d) => (
                <div key={d}>{d}</div>
              ),
            )}
...
function SOCKSDialog({
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <form
        onSubmit={submit}
        className="panel panel-pad w-full max-w-md space-y-3"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-base font-semibold">
          {editing ? "Edit SOCKS inbound" : "New SOCKS inbound"}
        </h2>
        <div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button
            ...
            disabled={pending || !name || !username || !password}
          >
