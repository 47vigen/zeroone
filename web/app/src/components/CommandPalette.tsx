...
export default function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    summary?.user_items?.forEach((u) =>
      list.push({
        ...
        label: u.email,
        group: "Users",
        action: () => navigate(`/users?q=${encodeURIComponent(u.email)}`),
      }),
    );
  ...
  return (
    <div
      className="fixed inset-0 z-50 grid place-items-start bg-black/40 px-4 pt-24"
      onClick={onClose}
    >
      <div
